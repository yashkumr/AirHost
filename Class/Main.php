 ...
 */
define("ADMINPATH","https://florib.in/bhartiyaonline/admin/admin/");
define("USERPATH","http://localhost/GitHub/question_project/admin/");
...
class MysqliDb
{
    ...
    {
       $login_error = array();
        if(!empty($_POST['token_'] == $_SESSION['token_']))
        {
           
            if(!empty($_POST['_email']))
            {
                $this->where ("admin_email",$_POST['_email']);
                $this->where ("admin_pass",$_POST['_pass']);
                $users = $this->get ("admin");
                if ($this->count > 0){
                    
                    $_SESSION['SESS_USER_ID'] = $users[0]['id'];
                    $_SESSION['SESS_ADMIN_ALL'] = $users;
                    $_SESSION['SESS_FIRST_NAME'] = $users[0]['admin_name'];
                    $this->redirect('dashboard.php','2');

                }else{
                    $login_error[] = 'Wrong Credentials Please Try again';
                }
            }else{
                $login_error[] = 'Please fill all details';
            }

        }else
        {
                $login_error[] = 'Page Expire';
        }
        return $login_error;
    }
...
function loader($class_color,$msg){ ?>
    <div class="container" align="center" top="70px" style="padding:100px">
        <button class="btn btn-<?=$class_color?>" disabled >
...
<?php }
    ...
    public function login_check_admin()
    {
        ...
        if( empty($_SESSION['SESS_USER_ADMIN']) )
        {
    ...
    public function login_check_user()
    {
        ...
        if(empty($_SESSION['SESS_USER_ID']) && empty($_SESSION['SESS_FIRST_NAME']))
        {
            $this->redirect('../index.php','0');
            exit() ;          
        }        
    ...
    // get random No 
    public function get_ramdon($table,$colom_name=NULL)
    {
            
            $random = rand(100000,111111);                           
            $where = array($colom_name => $random);
            
            $sql            = $this->UNI_Fetch($table,$where);
            $count_row      = $this->Row_count($sql);                 
            ...
            {
                 $this->get_ramdon($table,$colom_name);
            }
